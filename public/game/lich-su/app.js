...
function nodeState(node, idx, nodes){
  if(S.done[node.id]) return "done";
  if(!node.authored) return "locked";
  const prev = nodes[idx-1];
  if(idx===0 || (prev && (S.done[prev.id] || !prev.authored === false))) {
    if(idx===0) return "current";
    return S.done[prev.id] ? "current" : "locked";
  }
  return "locked";
}
...
function renderHome(){
  ...
  g.chapters.forEach(ch=>ch.nodes.forEach((n,i)=>{ if(!firstPlayable && nodeState(n,i,ch.nodes)==="current") firstPlayable=n; }));
  ...
  g.chapters.forEach((ch, ci)=>{
    ...
    ch.nodes.forEach((n,i)=>{
      const state = nodeState(n,i,ch.nodes);
