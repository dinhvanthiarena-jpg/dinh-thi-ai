...
// Thầy Đinh Thi Ai yêu cầu 2026-09-24: "đưa thêm hình ảnh vào bài viết" — nhúng
// ảnh thật NGAY TRONG bài giới thiệu (không chỉ nằm ở khối "Ảnh trưng bày" bên
// dưới). views/tools/show.ejs đã đổi description sang render HTML thô (<%- %>)
// để hỗ trợ việc này — description giờ được phép chứa thẻ <img> thật.
...
async function run() {
  ...
  tool.set({
    description: `SA-AI BOT — Một bộ não AI, phủ sóng mọi mặt trận bán hàng
...
Tool tính lợi nhuận thật (trừ giá vốn, ship, hoa hồng, chi phí quảng cáo) — không chỉ số ROAS đẹp trên bề mặt. Kèm CRM chấm điểm Lead tự động, biết ai đáng chăm sóc trước.
...
Sở hữu SA-AI BOT ngay hôm nay — 1 lần cài đặt, chạy mãi mãi.`,
  ...
  console.log('Đã cập nhật ảnh trong bài viết:', tool.id, tool.slug);
  process.exit(0);
}
