...
CÀ KHỊA CHUYỆN LƯỜI / TRÌ HOÃN:
 ...
 "Trì hoãn giỏi thế này chắc thi đấu SEA Games được đó!"
 "Cứ đà này chắc lập kỷ lục Guinness 'lười nhất vũ trụ' quá!"
 "Việc thì để đó, drama thì để đầu — đúng bài luôn ha!"
 "Ngày mai làm, ngày mai làm... rồi ngày mai lại thành hôm nay đó nha!"
 "Tốc độ này chắc rùa nó còn chạy trước cả cây số!"
 "Cái deadline nó đang khóc thét ngoài kia kìa, biết chưa?"
CÀ KHỊA CHUYỆN NGƠ NGƠ / MẤT TẬP TRUNG:
 ...
 "Sóng não đang roaming ở đâu vậy, kéo về đây coi!"
 "Tâm hồn treo ngược cành cây rồi hả, quay lại mặt đất đi!"
 "Ủa lag thiệt hay giả bộ lag vậy ta?"
 "Trí nhớ cá vàng ghé thăm hả, mới nói xong đã quên!"
...
// lượt trong cùng một cuộc gọi/phiên luyện nói — chỉ phần CHỐT (ngôn ngữ ép
// buộc, mở màn hay không, kết quả đọc) đổi theo từng lượt. Tách ra để đánh
// dấu cache_control: phần lặp lại chỉ bị tính ~10% giá token đầu vào từ lượt
// thứ 2 trở đi trong cùng phiên, thay vì tính đủ giá mỗi lượt như trước —
// không đổi một chữ nào trong nội dung prompt, chỉ đổi cách đóng gói khi gửi
// lên Anthropic.
...
async function reply({ history, level, words, mode, style }) {
  ...
  const res = await fetch(ANTHROPIC_API_URL, {
    ...
    body: JSON.stringify({
      ...
      system: toCacheableSystem(teach ? teachPrompt(level, words, heard)
